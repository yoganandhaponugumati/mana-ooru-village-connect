...
_Prepared against the live codebase (`mana-ooru-village-connect`, TanStack Start + React 19 + Supabase) and live deployment._
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **`role`** (permission tier)           | Controls _what you can do_: read, write, moderate, administer                                        | `profiles.role` enum: `citizen`, `village_admin`, `super_admin` (add `district_admin` later)                         |
| **`occupation`** (identity tag)        | Controls _what content is relevant to you_: Farmer sees crop prices, Doctor sees health-center tools | `profiles.occupation` — already exists, extend the CHECK constraint list to add Business Owner, NGO, Volunteer, etc. |
...
### 3.1 Government Schemes _(schema shipped this pass)_
...
### 3.2 Blood Donors _(schema shipped this pass)_
...
### 3.3 Gram Sabha / Village Polls _(schema shipped this pass)_
...
### 3.4 Village Budget / Development Tracker _(schema shipped this pass)_
...
_(Items 36–100 from the original brief — analytics dashboards, subscription tiers, machinery/vehicle/house marketplaces, sports, digital classes, training programs, etc. — are lower-leverage variations of the patterns already specified above: they reuse the `listings` table pattern, the `community_groups` pattern, or the storage-backed resource pattern from Tiers 2–3. Rather than list 65 more near-duplicate rows, apply the closest existing pattern from above when you're ready to build each one — that keeps the codebase consistent instead of accumulating 60 one-off table designs.)_
