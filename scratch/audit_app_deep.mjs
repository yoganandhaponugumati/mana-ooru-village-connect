...
files.forEach((filePath) => {
  ...
  [
    ...
    if (funcCallRegex.test(content)) {
      ...
      const isDecl = new RegExp(`(?:const|let|var|function|type|interface)\\s+${symbol}\\b`).test(
        content,
      );
  ...
  // React Hooks check
  ["useState", "useEffect", "useRef", "useMemo", "useCallback", "useContext", "useId"].forEach(
    (hook) => {
      const hookCallRegex = new RegExp(`\\b${hook}\\s*\\(`);
      if (hookCallRegex.test(content)) {
        const isImported = new RegExp(
          `import[\\s\\S]*?\\b${hook}\\b[\\s\\S]*?from\\s*["']react["']`,
        ).test(content);
