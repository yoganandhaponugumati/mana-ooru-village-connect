...
export const Route = createFileRoute("/profile")({
  head: () => ({ meta: [{ title: "Profile - GramMitra" }] }),
  component: () => (
      ...
      <main className="pb-24 pt-20 px-4 max-w-lg mx-auto flex-1 w-full">
        {!user && (
          <div className="mb-6 rounded-2xl bg-primary/10 p-5 border border-primary/20 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1 mb-4">
              Your profile builds trust across workers, land, marketplace, and services.
            </p>
            <AppLinkButton to="/auth" className="w-full">
              Sign In / Register
            </AppLinkButton>
          </div>
        )}
        ...
        <div className="mt-4 flex flex-col gap-3">
          ...
          <SurfaceCard className="divide-y divide-border/60">
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                <div className="grid size-8 place-items-center rounded-full bg-secondary/10 text-secondary">
              ...
              >
                <span
                  className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-all ${darkMode ? "left-6" : "left-1"}`}
                />
              </button>
            </div>
            <div className="flex items-center justify-between p-4">
              ...
              >
                <span
                  className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-all ${notificationsEnabled ? "left-6" : "left-1"}`}
                />
              </button>
            </div>
            <div className="flex items-center justify-between p-4">
          ...
          <SurfaceCard className="overflow-hidden">
            ...
            <button
              onClick={() => togglePanel("saved")}
              className="flex w-full items-center justify-between p-4 text-left hover:bg-muted/30 transition border-t border-border/60"
            >
              <div className="flex items-center gap-3">
                <div className="grid size-8 place-items-center rounded-full bg-primary/10 text-primary">
                  ...
                  <div className="divide-y divide-border">
                    {savedPosts.map((post) => (
                      <div key={post.id} className="flex items-center justify-between gap-3 py-3">
                        <div>
                          <p className="text-sm font-semibold text-clay line-clamp-1">
                            {post.title}
                          </p>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="grid size-8 place-items-center rounded-full bg-destructive/10 text-destructive">
