...
export const Route = createFileRoute("/transport")({
  head: () => ({ meta: [{ title: "Transport - GramMitra" }] }),
...
function TransportPage() {
  ...
  return (
    <PageLayout
      title="Transport & Machines"
      subtitle="Book tractors, autos, mini trucks, harvest machines, rotavators, and JCB support locally."
      icon={<Truck className="size-7" />}
    >
      <SectionHeader
        eyebrow="Village mobility"
        title={`Book transport near ${profile.village}`}
        description="Every option supports one-click call and a simple booking calendar view."
      />
        ...
        {transportOptions.map((item) => (
          <SurfaceCard key={item.id} className="p-5">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Available around {profile.village}. Confirm timing directly with the provider.
            </p>
            ...
            <a
              ...
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground"
            >
