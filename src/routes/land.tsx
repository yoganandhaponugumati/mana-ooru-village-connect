...
function LandPage() {
  ...
  const displayItems = items.length > 0 ? items : fallbackListings.filter((item) => item.type === "land");
  ...
  return (
    <PageLayout title="Lease Farmland" subtitle="List your land or find fields available this season." icon={<Wheat className="size-7" />}>
      <SectionHeader
        ...
        actions={<AppButton variant="primary" icon={<Plus className="size-4" />} onClick={() => setShowForm((v) => !v)}>{showForm ? "Cancel" : "List your land"}</AppButton>}
      />
      ...
      <SurfaceCard className="mb-8 overflow-hidden p-0">
        <div className="grid gap-0 lg:grid-cols-[1fr_1.3fr]">
          <div className="bg-primary p-7 text-primary-foreground">
            ...
            <p className="mt-2 text-sm leading-7 text-white/80">Compare water source, soil type, access roads, nearby landmarks, and owner contact before calling.</p>
          </div>
      ...
      {displayItems.length === 0 ? (
        <EmptyState
          ...
          action={<AppButton variant="primary" icon={<Plus className="size-4" />} onClick={() => setShowForm(true)}>List your land</AppButton>}
        />
      ) : (
