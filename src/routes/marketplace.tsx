...
function MarketPage() {
  ...
  return (
    <PageLayout
      ...
      subtitle="Buy & sell directly with neighbours — no middlemen."
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {CATS.map((c) => (
          <button
            key={c}
            onClick={() => setCat(c)}
            className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition ${cat === c ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:border-primary hover:text-primary"}`}
          >
            {c}
          </button>
        ))}
      ...
      {showForm && (
        <SurfaceCard className="mb-8 p-6 sm:p-8">
          <ListingForm
            ...
            photoLabel="Add product photo"
            photoHint="Show the item clearly so buyers can judge freshness, quantity, and quality."
            fields={[
              {
                name: "title",
                label: "Item name & quantity",
                placeholder: "e.g. Fresh tomatoes — 30 kg",
                required: true,
              },
              {
                name: "category",
              ...
              },
              {
                name: "description",
                label: "Description",
                placeholder: "Freshness, quality, terms…",
                textarea: true,
              },
