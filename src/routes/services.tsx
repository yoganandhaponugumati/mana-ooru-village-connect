...
import { Cable, Car, Drill, Hammer, Paintbrush, Plus, Router, ShowerHead, Tractor, Truck, Waves, Wrench } from "lucide-react";
...
function ServicesPage() {
  ...
  const displayItems = items.length > 0 ? items : fallbackListings.filter((item) => item.type === "service");
  ...
  return (
    <PageLayout title="Local Services" subtitle="Tractors, repairs, transport — book trusted local providers." icon={<Wrench className="size-7" />}>
      <SectionHeader
        ...
        actions={<AppButton variant="primary" icon={<Plus className="size-4" />} onClick={() => setShowForm((v) => !v)}>{showForm ? "Cancel" : "Offer a service"}</AppButton>}
      />
      <div className="mb-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {services.map((service) => (
          <SurfaceCard key={service.label} className="p-4">
            <button className="flex w-full items-center gap-3 text-left" onClick={() => setShowForm(false)}>
      ...
      {displayItems.length === 0 ? (
        <EmptyState
          ...
          action={<AppButton variant="primary" icon={<Plus className="size-4" />} onClick={() => setShowForm(true)}>Offer a service</AppButton>}
        />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {displayItems.map((i) => <ListingCard key={i.id} item={i} onDelete={items.length > 0 ? remove : undefined} />)}
        </div>
      )}
    </PageLayout>
  );
